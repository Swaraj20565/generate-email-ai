...
/**
 * -------------------------------
 * Route 2: Ask QA / Context-based answers
 * -------------------------------
 */
app.post("/api/ask", async (req, res) => {
  ...
  try {
    // Dummy vectorStore and cosineSim (replace with your actual embeddings logic)
    const vectorStore = []; // Example: [{id:1,text:"...", embedding:[...]}]
    const cosineSim = (a, b) => 0.9; // Replace with actual cosine similarity calculation

    // Create embedding for vector search (replace with actual embedding function)
    const createEmbedding = async (text) => {
      return Array(768).fill(0); // Dummy embedding
    };
    ...
    if (keyPoints && keyPoints.length)
      prompt += `Include key points: ${keyPoints.join(", ")}\n`;
